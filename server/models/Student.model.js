...
const studentSchema = new mongoose.Schema({
  // Existing User fields from User.js
  firstName: {
    type: String,
  ...
  role: {
    type: String,
    enum: {
      values: ['student', 'instructor', 'departmentHead', 'registrar', 'itAdmin', 'president', 'graduationCommittee'],
      message: 'Role must be one of: student, instructor, departmentHead, registrar, itAdmin, president, graduationCommittee'
    },
    default: 'student'
  },
  mustChangePassword: {
  ...
  profilePicture: {
    type: String // File path for uploaded picture
  },
...
studentSchema.statics.generateInstitutionalEmail = async function (firstName, fatherName) {
...
studentSchema.statics.generateTempPassword = function () {
...
studentSchema.methods.checkGraduationEligibility = async function () {
  try {
...
studentSchema.methods.markAsGraduated = async function (approvedBy, comments) {
...
studentSchema.index({ department: 1, currentYear: 1, currentSemester: 1 });
studentSchema.index({ probation: 1 });
studentSchema.index({ dismissed: 1 });
studentSchema.index({ status: 1 });
studentSchema.index({ isGraduated: 1 });
studentSchema.index({ clearanceStatus: 1 });
studentSchema.index({ finalProjectStatus: 1 });
